...
var i18n = function (options) {
 ...
 return {
   ...
   //@lblanch addition
    isJSONLoaded(locale, domain) {
      if(!locale) 
        throw new Error('You must provide a locale!');
      
      if ('string' !== typeof locale )
        throw new Error('Invalid argument, locale must be a string');

      if (!_dictionary[domain || defaults.domain]) {
        return false;
      } else {
        return !!_dictionary[domain || defaults.domain][locale]
      }
   },
