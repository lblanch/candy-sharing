...
/*function drawCandyBag(fallingTime) {
    ...
    for (var row = 0; row < CANDY_BAG_ROWS; row++) {
        for (var col = 0; col < CANDY_BAG_COLS; col++) {
            ...
            if (auxCandy != null) {
                if (fallingCandiesAfterEx[col] != null) {
                    if(bagIndex <= fallingCandiesAfterEx[col]) {
                        if(candyBag[fallingCandiesAfterEx[col]+CANDY_BAG_COLS] == null) {
                            if (candyBag[fallingCandiesAfterEx[col]].is)
                        }
                        console.log(bagIndex + " < " + fallingCandiesAfterEx[col] + " (" + candyBag[bagIndex].candyType+"_"+candyBag[bagIndex].candyColor+")");
                        auxCandy.isMoved = false;
                        auxCandy.isFalling = true;
                    }
                }
                if(fallingTime) {
                    if(auxCandy.isFalling) {
                        console.log(auxCandy.candyType + "_" + auxCandy.candyColor + " at " + bagIndex + " (" + auxCandy.isMoved + ")");
                        if (auxCandy.isMoved) {
                            auxCandy.draw(drawAtX, drawAtY);
                            auxCandy.isMoved = false;
                        } else {
                            //we are at the last row
                            if (row == CANDY_BAG_ROWS - 1) {
                                console.log(auxCandy.candyType + "_" + auxCandy.candyColor + " at " + bagIndex + " reached last row");
                                auxCandy.isFalling = false;
                                auxCandy.draw(drawAtX, drawAtY);
                            } else if (candyBag[bagIndex+CANDY_BAG_COLS] == null) {
                                //if position under is empty, move the falling candy
                                console.log(auxCandy.candyType + "_" + auxCandy.candyColor + " at " + bagIndex + " is moved to pos " + bagIndex+CANDY_BAG_COLS);
                                auxCandy.isMoved = true;
                                candyBag[bagIndex+CANDY_BAG_COLS] = auxCandy;
                                candyBag[bagIndex] = auxCandy.movedHere;
                                auxCandy.movedHere = null;
                            } else if(candyBag[bagIndex+CANDY_BAG_COLS].isFalling) {
                                //candy under is also falling
                                console.log(auxCandy.candyType + "_" + auxCandy.candyColor + " at " + bagIndex + " candy below is also is moved to pos " + bagIndex+CANDY_BAG_COLS);
                                auxCandy.isMoved = true;
                                candyBag[bagIndex+CANDY_BAG_COLS] = auxCandy;
